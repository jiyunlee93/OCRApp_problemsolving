...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        // 새로운 테이블 생성
        /* 이름은 TESTPAPER이고, 자동으로 값이 증가하는 _id 정수형 기본키 컬럼과
        contents 문자열 컬럼, page 정수형 컬럼, create_at 문자열 컬럼으로 구성된 테이블을 생성. */
    ...
    public void update(String contents, int page) {
        SQLiteDatabase db = getWritableDatabase();
        // 입력한 항목과 일치하는 행의 가격 정보 수정
        db.execSQL("UPDATE MONEYBOOK SET price=" + page + " WHERE item='" + contents + "';");
        db.close();
    }
    ...
    public void delete(String contents) {
        SQLiteDatabase db = getWritableDatabase();
        // 입력한 항목과 일치하는 행 삭제
        db.execSQL("DELETE FROM MONEYBOOK WHERE item='" + contents + "';");
        db.close();
    }
